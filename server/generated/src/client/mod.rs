    ...
    C: Has<XSpanIdString>  + Clone + Send + Sync + 'static,
{
        ...
        context: &C) -> Result<BookmarksGetResponse, ApiError>
    {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = serde_json::from_str::<models::BookmarkGetOkResponse>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(BookmarksGetResponse::OkResponse
                    (body)
                )
            }
            code => {
    ...
    async fn bookmarks_post(
        &self,
        param_bookmark_post_request_body: models::BookmarkPostRequestBody,
        context: &C) -> Result<BookmarksPostResponse, ApiError>
    {
        ...
        let body = serde_json::to_string(&param_bookmark_post_request_body).expect("impossible to fail to serialize");
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<models::BookmarkPostOkResponse>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(BookmarksPostResponse::OkResponse
                    (body)
                )
            }
            code => {
